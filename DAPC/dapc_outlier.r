...
for(i in 1:nrow(myMat)){
  grp <- find.clusters(data, n.pca = 32, choose.n.clust = FALSE,  max.n.clust = maxK)
  myMat[i,] <- grp$Kstat
}
...
#pramx <- xvalDapc(tab(data, NA.method="mean"), pop(data), n.pca.max=40, parallel = "multicore", ncpus = 4)
...
#by_year <- dapc(data, pop = metadata$year, perc.pca = 85, n.da = 3)
#scatter(by_year)
...
dapc_plot <- df %>% 
  ...
  labs(title = "DAPC (outlier markers)") +
...
ggsave("DAPC_outlier.png", height = 6, width = 11, units = "in")
